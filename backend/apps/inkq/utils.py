...
def analyze_image(img: Image, dict_of_vars: dict):
    ...
    prompt = (
        f"You have been given an image with some mathematical expressions, equations, or graphical problems, and you need to solve them. If expression can't be solved return formula if exists"
        ...
        f"6. if expression like x^2 + y^2, x^3 + y^3, etc: In this case, return appropriate related formula like (x + y)^2 - 2 * x * y for x^2 + y^2 or simplifyor evaluate the algebric expression 1/x + 1/y = (x + y)/ x * y{{'assign': True}}, keeping the variable as 'expr' and the value as string 'result' in the original dictionary. Include as many dicts as there are variables. RETURN AS A LIST OF DICTS. "
        f"7. You will be given an open end question like what is Car, what is date today, Who is Virat Kolhi, etc: In this case, return a short answer in string within 20 words strictly{{'assign': false}}, keeping the variable as 'expr' and the value as string 'result' in the original dictionary."
        ...
        f"note : Here is a dictionary of user-assigned variables. If the given expression has any of these variables, use its actual value from this dictionary accordingly: {dict_of_vars_str}. "
    ...
    print("json text : " , json_string)
    answers = []
    try:
        answers = ast.literal_eval(json_string)
